...
export default function ProfilePage() {
        ...
        if (preferences.platform_urls?.length) {
          setPlatforms(
            preferences.platform_urls.map((source) => ({
              name: source.name || source.platform_name || '新监测源',
  ...
  const addPlatform = () => {
    setPlatforms((current) => [...current, { name: '新监测源', url: 'https://', frequency: '10 分钟', status: '正常' }]);
  };
    ...
    <AppShell>
      <section className="profile-console">
        <div className="profile-heading">
          ...
          <h1>个人设置</h1>
          <p>设置监测源、关注领域和关键词，用于筛选后续推送的事件。</p>
        </div>
        ...
        <section className="profile-console-grid">
          ...
          <PreferenceEditor title="关注领域和关键词" areas={areas} setAreas={setAreas} keywords={keywords} setKeywords={setKeywords} />
        </section>
        ...
        <article className="source-config-card">
          <div className="section-heading">
            <div>
              ...
              <h2>监测平台和网址</h2>
              <p>填写要监测的网站或平台，并设置更新频率和运行状态。</p>
            </div>
            <button className="icon-action" type="button" onClick={addPlatform} aria-label="新增监测源">
              <Plus size={18} />
            </button>
          </div>

          <div className="source-list grouped">
            <SourceGroup title="运行正常" items={normalPlatforms} updatePlatform={updatePlatform} removePlatform={removePlatform} />
            <SourceGroup title="限流或异常" items={limitedPlatforms} updatePlatform={updatePlatform} removePlatform={removePlatform} empty="没有限流或异常的监测源" />
          </div>
        </article>
...
function SourceGroup({ title, items, updatePlatform, removePlatform, empty = '暂无监测源' }) {
  return (
      ...
      {items.length === 0 ? (
        ...
        items.map(({ platform, index }) => (
          <div className="source-config-row" key={`${platform.name}-${index}`}>
            ...
            <button type="button" onClick={() => removePlatform(index)} aria-label="删除监测源">
              <X size={17} />
            </button>
          </div>
        ))
...
function PreferenceEditor({ title, areas, setAreas, keywords, setKeywords }) {
  ...
  return (
    <article className="preference-console-card">
      <div className="section-heading">
        <div>
          <span className="eyebrow">关注规则</span>
          <h2>{title}</h2>
          <p>包含这些领域或关键词的事件，会优先出现在看板和推送中。</p>
        </div>
      </div>
      ...
      <div className="preference-add-row">
        ...
        <input value={draft} onChange={(event) => setDraft(event.target.value)} placeholder="输入关键词或领域" />
